 ...
 * describe software nobody runs. `probe-fixture.json` is that library as the
 * probe's own loader sees it, and the last block asserts this implementation
 * reaches the verdicts `analyze.py` printed for it.
 *
...
describe('agreement with the probe', () => {
   ...
   *
   * `probe-fixture.json` is the output of `tools/geotag-probe`'s own loader
   * over its own synthetic library — 83 photos, generated by
   * `make_fixture.py`, read by `analyze.py`'s `load()` and dumped verbatim.
   * Using the probe's extraction rather than a hand-rolled one matters: an
   * earlier attempt with a separate exiftool call lost the two PNG
   * screenshots, because they carry no DateTimeOriginal and the probe falls
   * back to FileModifyDate. A fixture built differently from the thing it is
   * checking proves nothing.
   *
   * The point of the probe is to measure, against real camera rolls, how
   * often this finds a clean event. That measurement describes software
   * nobody runs if the TypeScript has drifted from the Python, so the numbers
   * below are the numbers `analyze.py` printed for this library:
   *
   *   2026-07-18T20:05   43  100%   93%   38m   pre-select 40
   *   2026-07-25T13:00   18    0%    0%     -   SHOW, TICK NOTHING
   *   2026-08-01T18:00   20   50%  100%   33m   SHOW, TICK NOTHING
   *
   * To regenerate after changing the fixture generator, see the README.
   */
  const library = JSON.parse(
    readFileSync(new URL('./probe-fixture.json', import.meta.url), 'utf8'),
  ) as Candidate[];

  const verdicts = () =>
    sessionise(library).map((session) => {
      const suggestion = narrow(session.photos, {
        start: session.start,
        end: session.end,
      });
      const geotagged = suggestion.candidates.filter((c) => c.lat !== null).length;
      return {
        n: suggestion.candidates.length,
        gps: Math.round((100 * geotagged) / suggestion.candidates.length),
        preselected: suggestion.preselected.length,
        confidence: suggestion.confidence,
        screenshots: suggestion.screenshotsExcluded,
      };
    });

  it('finds the same three sessions', () => {
    // Time-gap splitting only, so this holds in any timezone: the boundaries
    // come from intervals between photos, never from a wall clock.
    expect(verdicts().map((v) => v.n)).toEqual([43, 18, 20]);
  });

  it('reaches the same verdict on each', () => {
    expect(verdicts()).toEqual([
      { n: 43, gps: 100, preselected: 40, confidence: 'high', screenshots: 2 },
      { n: 18, gps: 0, preselected: 0, confidence: 'low', screenshots: 0 },
      { n: 20, gps: 50, preselected: 0, confidence: 'low', screenshots: 0 },
    ]);
  });

  it('measures the same cluster spread', () => {
    // The one continuous number either implementation reports. Agreeing on
    // the verdict while disagreeing on the geometry would mean the two had
    // drifted and the thresholds were hiding it.
    const [first] = sessionise(library);
    const suggestion = narrow(first!.photos, { start: first!.start, end: first!.end });
    expect(suggestion.clusterSpreadM).toBe(38);
  });
});
